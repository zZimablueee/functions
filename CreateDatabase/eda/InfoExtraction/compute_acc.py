    ...
    while True:
        ...
        while not node.is_end():
            if node.move is not None:
                ...
                current_eval=score
                prev_evaluation = score  # 保持全局估值传递
        ...
        all_games.append( (avg_cp_white, final_acc_white, avg_cp_black, final_acc_black) )
    return all_games
...
def analyze_pgn(input_source, engine_path, threads, depth, is_verbose):
    ...
    all_games=process(input_source,engine,depth,is_verbose,board)
    ...
    for game_idx,(avg_cp_white,acc_white,avg_cp_black,acc_black) in enumerate(all_games,1):
        if is_verbose:
            print(f'\n—— Game{game_idx} ——')
            print(f'White:{avg_cp_white:.0f}cp loss, {acc_white:.0f}% accuracy')
            print(f"Black: {avg_cp_black:.0f}cp loss, {acc_black:.0f}% accuracy")
...
#python compute_acc.py 12 2 C:\\Users\\Administrator\\Desktop\\stockfish-windows-x86-64-avx2\\stockfish\\stockfish-windows-x86-64-avx2.exe -file=C:\\Users\\Administrator\\Desktop\\findcode\\input\\master_games.pgn -verbose
